...
export const LoginPage: React.FC = () => {
  ...
  const [email, setEmail] = useState('hari.prasath@placementportal.edu');
  ...
  const handleDemoStudentLogin = async () => {
    ...
    setRole('student');
    setIsLoading(true);
    await login('hari.prasath@placementportal.edu', 'student');
    navigate('/dashboard');
  };
  ...
  const handleDemoAdminLogin = async () => {
    ...
    setRole('admin');
    setIsLoading(true);
    await login('tpo.officer@placementportal.edu', 'admin');
    navigate('/dashboard');
  };
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="bg-gradient-to-r from-indigo-50 via-sky-50 to-indigo-50 border border-indigo-200/80 rounded-2xl p-4 shadow-xs">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-bold text-indigo-950">Quick 1-Click Demo Login</span>
          </div>
          <p className="text-[11px] text-slate-600 mb-3">
            Instant access to all modules without typing credentials:
          </p>
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={handleDemoStudentLogin}
              className="flex items-center justify-center gap-1.5 py-2 px-3 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl transition shadow-xs"
            >
